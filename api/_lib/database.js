...
const DATA_DIR = path.join(os.tmpdir(), 'andresweb-data');
...
try {
  ...
  console.warn('Aviso: Falha ao criar diretorio em /tmp:', err.message);
}
...
class Database {
  ...
  save() {
    try {
      ...
      console.warn('Salvando em memoria');
    }
  }
  ...
  filter(entityName, criteria = {}, sort = '-created_date', limit = 1000) {
    let items = this.getCollection(entityName);
    items = items.filter(item => this.matchCriteria(item, criteria));
    return this.applySortAndLimit(items, sort, limit);
  }
  ...
  bulkCreate(entityName, dataArray) {
    ...
    return dataArray.map(item => this.create(entityName, item));
  }
