...
import { RefreshCw, Plus, ArrowLeft, Search, ChevronDown, ChevronUp, Trash2, ArrowRight } from 'lucide-react';
...
export default function ExchangeForm({ onDone }) {
  ...
  const confirm = async () => {
    ...
    if (!newItems.length) { toast.error('Adicione ao menos uma nova peça'); return; }
    setSaving(true);
    try {
      const payload = {
        ...
        new_items: newItems.map(i => ({ product_id: i.product_id, product_name: i.product_name, variant_size: i.variant_size, variant_color: i.variant_color, quantity: i.quantity, unit_price: i.unit_price })),
      ...
      const msg = d > 0 ? `Troca concluída — cliente pagou ${BRL(d)}` : d < 0 ? `Troca concluída — devolver ${BRL(Math.abs(d))}` : 'Troca concluída — valor equilibrado';
      toast.success(msg);
      onDone?.();
    } catch (e) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      {/* Coluna esquerda: venda + devoluções */}
      <div className="space-y-5">
        <div className="bg-card border border-border rounded-2xl p-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground mb-3">1. Venda original</h3>
          {!selected ? (
            <>
            ...
            <div>
              ...
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Selecione as peças devolvidas:</p>
        ...
        {selected && (
          <div className="bg-card border border-border rounded-2xl p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground mb-3">2. Novas peças</h3>
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder="Buscar produto..." value={prodSearch} onChange={e => setProdSearch(e.target.value)} className="pl-9" />
            </div>
            <div className="border border-border rounded-xl max-h-56 overflow-y-auto">
              {filteredProds.map(p => {
                  ...
                  <div key={p.id} className="border-b border-border last:border-0">
                    <button type="button" onClick={() => setExpanded(e => ({ ...e, [p.id]: !e[p.id] }))} className="w-full flex items-center justify-between px-3 py-2 hover:bg-muted/40 text-left">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{p.sku || p.gtin || p.category} · {BRL(p.price)}</p>
                      </div>
      ...
      <div className="space-y-5">
        <div className="bg-card border border-border rounded-2xl p-5 sticky top-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground mb-4">Resumo da troca</h3>
          <div className="space-y-2.5 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Peças devolvidas</span><span className="font-medium">{returnedItems.reduce((s, i) => s + i.qty, 0)} un · {BRL(returned_value)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Novas peças</span><span className="font-medium">{newItems.reduce((s, i) => s + i.quantity, 0)} un · {BRL(new_value)}</span></div>
            <div className="border-t border-border pt-2.5 mt-2.5">
              <div className="flex justify-between items-center">
                ...
                <span className={cn("text-lg font-bold", difference > 0 ? "text-amber-600" : difference < 0 ? "text-emerald-600" : "text-muted-foreground")}>
                  {difference > 0 ? `Cliente paga ${BRL(difference)}` : difference < 0 ? `Devolver ${BRL(Math.abs(difference))}` : 'Equilibrado'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3">
            ...
            {difference < 0 && (
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold uppercase tracking-wide">Loja devolve via</Label>
          ...
          <Button onClick={confirm} disabled={saving || !selected || !returnedItems.length || !newItems.length} className="w-full mt-5 h-11 text-base">
            <RefreshCw className="w-4 h-4 mr-2" /> {saving ? 'Processando...' : 'Confirmar troca'}
          </Button>
          <p className="text-xs text-muted-foreground text-center mt-2">O estoque é ajustado automaticamente (entrada + saída).</p>
        </div>
      </div>
    </div>
  );
