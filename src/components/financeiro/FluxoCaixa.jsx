  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-card shadow-sm rounded-2xl border border-slate-200 dark:border-border p-5">
        <h3 className="font-serif text-base font-semibold mb-1 text-slate-900 dark:text-slate-100">Fluxo Diário — {format(new Date(), "MMMM 'de' yyyy", { locale: ptBR })}</h3>
        <p className="text-xs text-slate-500 dark:text-muted-foreground mb-4">Saldo acumulado ao longo do mês atual</p>
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={dailyData}>
            ...
            <Line dataKey="entrada" name="entrada" stroke="#059669" strokeWidth={1.5} dot={false} />
            <Line dataKey="saida" name="saida" stroke="#ef4444" strokeWidth={1.5} dot={false} />
      ...
      <div className="bg-white dark:bg-card shadow-sm rounded-2xl border border-slate-200 dark:border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200 dark:border-border">
          <h3 className="font-serif text-base font-semibold text-slate-900 dark:text-slate-100">Resumo Mensal (6 meses)</h3>
        </div>
        <table className="w-full">
          <thead>
            <tr className="bg-slate-50 dark:bg-muted/40 border-b border-slate-200 dark:border-border">
              <th className="text-left px-5 py-3 text-xs font-medium text-slate-500 dark:text-muted-foreground uppercase tracking-wide">Mês</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 dark:text-muted-foreground uppercase tracking-wide">Entradas</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 dark:text-muted-foreground uppercase tracking-wide">Saídas</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 dark:text-muted-foreground uppercase tracking-wide">Saldo</th>
            </tr>
          </thead>
          <tbody>
            {monthlyData.map((row, i) => (
              <tr key={i} className="border-b border-slate-200 dark:border-border last:border-0 hover:bg-slate-50 dark:hover:bg-muted/20 transition-colors">
                <td className="px-5 py-3 text-sm font-medium capitalize text-slate-900 dark:text-slate-100">{row.label}</td>
                <td className="px-4 py-3 text-sm text-right text-emerald-600 dark:text-emerald-500 font-medium">R$ {row.entrada.toFixed(2).replace('.', ',')}</td>
                <td className="px-4 py-3 text-sm text-right text-destructive font-medium">R$ {row.saida.toFixed(2).replace('.', ',')}</td>
                <td className={`px-4 py-3 text-sm text-right font-semibold font-serif ${row.saldo >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-destructive'}`}>
